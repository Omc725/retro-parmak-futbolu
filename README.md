<file name="README.md">
<div align="center">

  <!-- BURAYA KENDİ BANNER RESMİNİZİ KOYABİLİRSİNİZ. ŞİMDİLİK PROJE SİMGESİNİ KULLANIYORUM -->
  <img src="public/icon-512x512.avif" alt="Retro Parmak Futbolu Logosu" width="200"/>

  # Retro Parmak Futbolu

  **Atari salonu ruhunu cebinize getiren, hızlı, eğlenceli ve rekabetçi bir parmak futbolu oyunu!**

  [![Lisans: MIT](https://img.shields.io/badge/Lisans-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
  [![GitHub Yıldızları](https://img.shields.io/github/stars/Omc725/retro-parmak-futbolu?style=social)](https://github.com/Omc725/retro-parmak-futbolu/stargazers)

</div>

<p align="center">
  Hızlı bir maça atlayın, zorlu bir lig sezonunda şampiyonluk kovalayın veya eleme usulü bir turnuvada kupaya uzanın. Takımınızı seçin, yapay zekaya karşı yeteneklerinizi gösterin ve parmak futbolu efsanesi olun!
</p>

<div align="center">
  <a href="https://omc725.github.io/retro-parmak-futbolu/">
    <img src="https://img.shields.io/badge/►_OYUNU_OYNA-2E8B57?style=for-the-badge&logoColor=white" alt="Oyunu Oyna Butonu"/>
  </a>
</div>

---

## ✨ Özellikler

*   **Üç Farklı Oyun Modu:**
    *   🏆 **Hızlı Maç:** Anında aksiyona dalın.
    *   ⚽ **Lig Modu:** Takımınızla tam bir sezon boyunca mücadele ederek puan tablosunun zirvesine tırmanın.
    *   🥇 **Turnuva Modu:** Eleme usulü maçlarla kupaya giden yolda rakiplerinizi bir bir geçin.
*   **Gelişmiş Yapay Zeka:** Farklı zorluk seviyelerine sahip, akıllı ve rekabetçi yapay zeka rakipler.
*   **Geniş Takım Yelpazesi:** Dünya genelinden onlarca lisanssız, hayali takım arasından favorinizi seçin.
*   **Kapsamlı Özelleştirme:** Oyun deneyiminizi kişiselleştirmek için farklı saha ve top stilleri.
*   **PWA Desteği:** Oyunu cihazınıza bir uygulama gibi kurun ve internet olmadan da oynayın!

---

## 🎨 Özelleştirme Seçenekleri

Oyun deneyiminizi kişiselleştirmek için mevcut olan saha ve top stilleri:

### Saha Stilleri

| Saha 1 | Saha 2 | Saha 3 | Saha 4 |
| :---: | :---: | :---: | :---: |
| <img src="public/assets/fields/saha1.avif" width="200"> | <img src="public/assets/fields/saha2.avif" width="200"> | <img src="public/assets/fields/saha3.avif" width="200"> | <img src="public/assets/fields/saha4.avif" width="200"> |
| **Saha 5** | **Saha 6** | **Saha 7** | **Saha 8** |
| <img src="public/assets/fields/saha5.avif" width="200"> | <img src="public/assets/fields/saha6.avif" width="200"> | <img src="public/assets/fields/saha7.avif" width="200"> | <img src="public/assets/fields/saha8.avif" width="200"> |
| **Saha 9** | **Saha 10** | **Saha 11** | **Saha 12** |
| <img src="public/assets/fields/saha9.avif" width="200"> | <img src="public/assets/fields/saha10.avif" width="200"> | <img src="public/assets/fields/saha11.avif" width="200"> | <img src="public/assets/fields/saha12.avif" width="200"> |

### Top Stilleri

| Top 1 | Top 2 | Top 3 | Top 4 | Top 5 |
| :---: | :---: | :---: | :---: | :---: |
| <img src="public/assets/balls/top1.avif" width="80"> | <img src="public/assets/balls/top2.avif" width="80"> | <img src="public/assets/balls/top3.avif" width="80"> | <img src="public/assets/balls/top4.avif" width="80"> | <img src="public/assets/balls/top5.avif" width="80"> |
| **Top 6** | **Top 7** | **Top 8** | **Top 9** | **Top 10** |
| <img src="public/assets/balls/top6.avif" width="80"> | <img src="public/assets/balls/top7.avif" width="80"> | <img src="public/assets/balls/top8.avif" width="80"> | <img src="public/assets/balls/top9.avif" width="80"> | <img src="public/assets/balls/top10.avif" width="80"> |

---

## 📲 PWA Olarak Yükleme (Çevrimdışı Oynama)

Bu oyun, bir **Aşamalı Web Uygulaması (PWA)** olarak tasarlanmıştır. Bu, oyunu bir mağazadan indirmiş gibi cihazınıza kurabileceğiniz ve internet bağlantınız olmasa bile oynayabileceğiniz anlamına gelir.

### PWA Nedir?
PWA, modern web teknolojilerini kullanarak bir web sitesine uygulama benzeri özellikler (çevrimdışı çalışma, ana ekrana ekleme, anlık bildirimler vb.) kazandıran bir yöntemdir.

### Nasıl Yüklenir?

#### 📱 Mobil Cihazlarda (Android/iOS)

1.  Cihazınızın tarayıcısından (Chrome, Safari vb.) oyunun web sitesini ziyaret edin: **[https://omc725.github.io/retro-parmak-futbolu/](https://omc725.github.io/retro-parmak-futbolu/)**
2.  **Android (Chrome):** Adres çubuğunun sağ tarafında çıkan **"Yükle" (Install)** simgesine veya tarayıcı menüsündeki (⋮) **"Uygulamayı Yükle"** seçeneğine dokunun.
3.  **iOS (Safari):** Paylaşım simgesine (↑) dokunun ve açılan menüden **"Ana Ekrana Ekle" (Add to Home Screen)** seçeneğini seçin.

Artık oyun, diğer uygulamalarınızın yanında, ana ekranınızda bir simge olarak görünecektir.

#### 💻 Masaüstünde (Chrome/Edge)

1.  Tarayıcınızdan oyunun web sitesini ziyaret edin.
2.  Adres çubuğunun sağ tarafında, yer imi yıldızının yanında bir **bilgisayar ve indirme oku simgesi** göreceksiniz.
3.  Bu simgeye tıklayın ve ardından **"Yükle" (Install)** butonuna basın.

Oyun, kendi penceresinde açılan bir masaüstü uygulaması olarak yüklenecektir.

---

## 🚀 Yerel Olarak Çalıştırma (Geliştiriciler İçin)

Bu projeyi kendi bilgisayarınızda geliştirmek veya çalıştırmak isterseniz, aşağıdaki adımları izleyin.

### Gereksinimler
*   [Node.js](https://nodejs.org/) (v18 veya üstü tavsiye edilir)
*   npm (Node.js ile birlikte gelir)

### Kurulum ve Başlatma

1.  **Depoyu klonlayın:**
    ```bash
    git clone https://github.com/Omc725/retro-parmak-futbolu.git
    ```

2.  **Proje dizinine gidin:**
    ```bash
    cd retro-parmak-futbolu
    ```

3.  **Gerekli paketleri yükleyin:**
    ```bash
    npm install
    ```

4.  **Geliştirme sunucusunu başlatın:**
    ```bash
    npm run dev
    ```

5.  Tarayıcınızda `http://localhost:3000` adresini açın. Proje canlı olarak çalışmaya başlayacaktır.

### 🛠️ Kullanılabilir Scriptler

*   `npm run dev`: Geliştirme sunucusunu başlatır.
*   `npm run build`: Projeyi `dist` klasörüne yayınlanmaya hazır hale getirir.
*   `npm run deploy`: Projeyi derler ve GitHub Pages'e otomatik olarak yayınlar.
*   `npm run optimize-assets`: `public/assets` klasöründeki tüm resimleri optimize eder, eskilerini siler ve koddaki dosya yollarını günceller.

---

## 💻 Kullanılan Teknolojiler

*   **React:** Kullanıcı arayüzü için.
*   **TypeScript:** Daha güvenli ve ölçeklenebilir kod yazımı için.
*   **Vite:** Hızlı geliştirme ve build işlemleri için.
*   **Tailwind CSS:** Hızlı ve modern arayüz tasarımı için.
*   **Vite PWA Plugin:** Projeyi çevrimdışı çalışabilen bir PWA'ya dönüştürmek için.
*   **Sharp & Glob:** Resim optimizasyon otomasyonu için.
</file>
